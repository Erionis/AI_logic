...
def sentence1() -> Expr:
    ...
    expr_1 = A|B
    expr_2 = (~A) % ((~B)|C)
    expr_3 = disjoin([(~A),(~B),(C)])
    expr_final = conjoin([expr_1,expr_2,expr_3])
...
def sentence2() -> Expr:
    ...
    expr_3 = (~(B&(~C))) >> A
    expr_4 = (~D) >> C
    expr_final = conjoin([expr_1,expr_2,expr_3,expr_4])
...
def sentence3() -> Expr:
    ...
    expr_1 = PacmanAlive_1 % ((PacmanAlive_0 & (~PacmanKilled_0)) | (~(PacmanAlive_0) & PacmanBorn_0))
    expr_2 = ~(PacmanAlive_0 & PacmanBorn_0)
    expr_3 = PacmanBorn_0
    expr_final = conjoin([expr_1,expr_2,expr_3])
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    neg_conclusion = Expr('~', conclusion)
    sentence = conjoin([premise, neg_conclusion])
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    # Generate all pairs of literals and create a clause for each pair
    clauses = [disjoin([~l1, ~l2]) for l1, l2 in combinations(literals, 2) if l1 != l2]

    # # Return the conjunction of the clauses
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    # If a wall is at (x, y) --> Pacman is not at (x, y)
    wall_axioms = [PropSymbolExpr(wall_str, x, y) >> ~PropSymbolExpr(pacman_str, x, y, time=t) for x, y in all_coords]
    pacphysics_sentences.append(conjoin(wall_axioms))       
    # Pacman is at exactly one of the squares at timestep t.
    pac_axioms = [PropSymbolExpr(pacman_str, x, y, time=t) for x, y in non_outer_wall_coords]
    pacphysics_sentences.append(exactlyOne(pac_axioms))
    # Pacman takes exactly one action at timestep t.   
    action_axioms = [PropSymbolExpr(action, time=t) for action in DIRECTIONS]
    pacphysics_sentences.append(exactlyOne(action_axioms))
...
def positionLogicPlan(problem) -> List:
    ...
    # Add to KB: initial knowledge: Pacman is at (x0, y0) at time 0
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
    
    for t in range(50):
        print(f"t = {t}")
        
        # Add to KB: Pacman can only be at one place at a time
        pac_loc_axioms = [PropSymbolExpr(pacman_str, x, y, time=t) for x, y in non_wall_coords]
        KB.append(exactlyOne(pac_loc_axioms))
        
        # find a model where Pacman is at (xg, yg) at time t
        model = findModel(conjoin([PropSymbolExpr(pacman_str, xg, yg, time = t), conjoin(KB)]))
        if model:
            return extractActionSequence(model, actions)
        
        # Add to KB: Pacman takes an action at time t
        pac_action_axioms = [PropSymbolExpr(action, time=t) for action in actions] 
        KB.append(exactlyOne(pac_action_axioms))
        
        # Add to KB Transition Model senteces
...
def foodLogicPlan(problem) -> List:
    ...
    # Initialize KB with food locations
    for x, y in food:
    ...
    for t in range(50):
        ...
        # Generate a goal assertion that there is no more food on the map
        food_pos = [PropSymbolExpr(food_str, x, y, time=t) for (x,y) in food]
        goal = ~disjoin(food_pos)
        
        # Check if there exists a sequence of actions that satisfies the conjunction of the KB and the goal
        model = findModel(conjoin([conjoin(KB), goal]))
        if model:
            return extractActionSequence(model, actions)
        ...
        for x, y in non_wall_coords:
            ...
            food_curr_pos = PropSymbolExpr(food_str, x, y, time=t)
            food_next_pos = PropSymbolExpr(food_str, x, y, time=t+1)
            # Food's presence or absence at (x, y) at time t+1 depends on whether Pacman is at (x, y) at time t
            KB.append(~food_next_pos % (pacman_pos | ~food_curr_pos))
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        # Add to KB pacphysicsAxioms at time t
        KB.append(pacphysicsAxioms(t, all_coords, non_outer_wall_coords, walls_grid, sensorAxioms, allLegalSuccessorAxioms))
        
        # Add to KB all possible actions at time t
        KB.append(PropSymbolExpr(agent.actions[t], time= t))
        
        # add to KB percept information at time t
        KB.append(fourBitPerceptRules(t = t, percepts = agent.getPercepts()))
        
        # find possible locations at time t with updated KB
        possible_locations = []
        for (x, y) in non_outer_wall_coords:
            pacman_pos = PropSymbolExpr(pacman_str, x, y, time= t)
            if findModel(conjoin([conjoin(KB), pacman_pos])):
                possible_locations.append((x, y))
                
            if entails(conjoin(KB), pacman_pos):
...
def mapping(problem, agent) -> Generator:
    ...
    start_loc = PropSymbolExpr(pacman_str, pac_x_0, pac_y_0, time = 0)
    KB.append(start_loc)
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        # add to KB percept information at time t
        KB.append(numAdjWallsPerceptRules(t = t, percepts = agent.getPercepts()))     
